...
- 두 로그인 ID와 관리자로 지정한 ID는 Git에서 제외된 `server/.env`와 로컬 DB에만 둡니다. 공개 예시의 `ACCOUNT_USERNAMES`와 `ADMIN_USERNAME`은 의도적으로 비어 있으며, 계정 설정이 없거나 기존 DB와 다르면 서버는 계정을 임의로 추가하지 않고 시작을 중단합니다. 관리자 설정이 없거나 두 계정 중 하나와 정확히 일치하지 않으면 관리자 API만 닫힌 상태로 유지됩니다.
...
기존 두 계정 중 하나를 관리자로 지정하려면 이 PC의 대화형 WSL 터미널에서 한 번 실행합니다.
...
비밀번호가 설정된 계정이 하나뿐이면 그 계정을 선택합니다. 현재처럼 두 계정이 모두 활성화된 경우 관리자로 쓸 ID를 묻지만 입력 문자는 화면과 셸 기록에 표시하지 않습니다. 실제 ID를 입력하지 않고 `ACCOUNT_USERNAMES` 순서로 고르려면 명령 끝에 `--position first` 또는 `--position second`를 붙여도 됩니다. 자동으로 어느 쪽인지 추정하지 않으며, 선택한 ID는 Git에서 제외된 권한 `0600`의 `server/.env`에만 `ADMIN_USERNAME`으로 저장됩니다.

관리자 계정으로 로그인하면 **관리자** 화면이 나타납니다. 여기서 다음을 확인하거나 조작할 수 있습니다.

- 서버 가동 시간, 모델 상태, CPU load, RAM·서버 프로세스 메모리, 디스크와 사용 가능한 GPU 메모리
- 두 계정의 활성화 여부, 현재 접속 여부와 `기록 중·업로드 중·변환 중·후보정 중·자리 비움` 같은 최소 활동 상태, 활성 세션 수와 작업 대기열
