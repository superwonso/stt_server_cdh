...
`server/.env`를 이 컴퓨터에서만 열어 `ACCOUNT_USERNAMES`에 쉼표로 구분한 두 로그인 ID를 입력하세요. ID는 소문자 영문·숫자·점·밑줄·하이픈으로 된 1~32자여야 하며 서로 달라야 합니다. AI 후보정을 쓸 때는 같은 파일의 `MINDLOGIC_API_KEY`에 NOVA Gateway 키를 넣고, `MINDLOGIC_BASE_URL`과 `MINDLOGIC_MODEL=solar-pro4`는 예시값 그대로 둡니다. 따옴표·`Bearer ` 접두사 없이 키 값만 넣습니다. 실제 값이 든 파일은 커밋하거나 다른 곳에 공유하지 마세요.
...
백업에는 두 계정의 비밀번호 해시, 개인 수업 텍스트와 음성이 들어갈 수 있습니다. GitHub, 공유 Drive, 메신저에 원본 그대로 올리지 마세요. 앱에서 수업을 삭제해도 먼저 만든 백업에서는 자동으로 지워지지 않습니다.
