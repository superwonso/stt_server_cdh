...
- 일시정지 상태는 로그인 세션을 대신하지 않습니다. 세션이 만료되거나 서버가 인증을 거절하면 다른 계정으로 음성이 섞이지 않도록 보안상 현재 녹음을 종료할 수 있습니다. 같은 탭에서 같은 계정으로 다시 로그인하면 메모리에 남은 마지막 음성 전송과 예약한 후보정을 이어가며, 다른 계정은 그 작업을 가져갈 수 없습니다. 그래도 새로고침하면 메모리의 미전송 음성은 사라지므로 먼저 화면의 저장·전송 상태를 확인하세요.
...
실제 Chrome/Edge와 Quick Tunnel에서 마이크·공유 소리를 장시간 일시정지/재개하거나 여러 차례 반복했을 때의 경계 음절, 3초 문맥, 메모리 증가, 세션 만료 순간의 마지막 청크는 아직 확인하지 않았습니다. 첫 입력이 50ms도 쌓이기 전에 일시정지하고 즉시 탭까지 종료하면 그 극히 짧은 PCM은 아직 서버로 보낼 수 없어 복구되지 않습니다. 중요한 수업 전에는 사용할 기기와 네트워크에서 짧은 일시정지·재개·종료 시험을 먼저 하세요.
