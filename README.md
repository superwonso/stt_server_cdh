...
- **NAVER CLOVA Speech**는 키가 서버에 설정돼 있으면 새 마이크 수업의 기본값입니다. 로그인 직후 서버 상태를 먼저 확인한 다음 선택하며, 마이크 음성은 이 PC와 Cloudflare를 거쳐 NAVER Cloud로 전송됩니다.
...
2026-09-04 현재 변경분에서 Python 서버/API/DB/설정/전사·가져오기·후보정·관리자·터널·CLOVA fake 계약 **152개**와 Node 앱 상태·오디오·파일 재개·정적 경계 **135개**가 모두 통과했습니다. Python compile, JavaScript 10개 파일 `--check`, `git diff --check`도 통과했습니다. CLOVA 계약에는 공식 영어 응답 형태의 띄어쓰기 보존과 한국어 overlap 부분 문자열 검사가 포함됩니다. Node 앱 대역은 실제 UUID·계정·provider·캡처 시간축·CLOVA `inflight`·마지막 조각 순서·영속 삭제와 CLOVA 우선 선택·Qwen fallback 계약을 검사하며, PCM이 전혀 없는 종료에 가짜 final WAV를 만들지 않습니다.
