...
- 수업을 연 뒤 형식을 고르고 **받아쓰기 저장**을 누르면 UTF-8 `.txt` 또는 제목·날짜·언어·타임스탬프가 포함된 `.md`로 내보냅니다.
...
- **수업 삭제**는 확인창을 거쳐 받아쓰기, 청크 정보, 연결된 파일 변환 기록과 보존 WAV를 함께 영구 삭제합니다. 진행 중인 전사나 파일 변환이 있으면 서버가 `409`로 거절하므로 먼저 끝내거나 취소하세요. 삭제한 내용은 앱에서 복구할 수 없지만, 삭제 전에 만든 별도 백업에는 남아 있을 수 있습니다.
