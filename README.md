...
최신 작업 트리에는 같은 탭의 최대 24시간 로그인 유지, 완료된 수업의 AI 요약, 전체 수업 문맥을 참고하는 한국어 번역과 Markdown 다운로드를 추가했습니다. 무음 자동 휴식은 사용자 요청에 따라 제거했으며, 조용해도 녹음을 계속하고 **수동 일시정지·재개**만 사용합니다. 최신 코드는 실제 Qwen·CLOVA의 짧은 공개 음성 시험과 격리된 실제 Chromium 시험을 수행했지만, 이 문서 정리 시점에는 최종 서버 재시작·Pages 배포 전입니다. 최종 배포 체크포인트와 전체 회귀 결과는 [HANDOFF.md](HANDOFF.md)를 확인하세요.
...
- 계정: 관리자에게 요청
- **이전 운영 배포에서 확인:** GitHub Pages 배포, Quick Tunnel 외부 `/health`, Pages CORS, 허용하지 않은 웹 출처 차단. 최신 작업 트리의 배포 완료를 뜻하지 않습니다.
