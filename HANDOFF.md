...
## 후속 개선 6단계 — 검증·공개 반영, 운영 API 재시작 전 (2026-09-06)

사용자가 제안 순서대로 진행하도록 요청했고, 복구 백업 대상으로 Google Drive가 아닌 **이 PC의 D 드라이브**를 선택했다. 이어서 커밋·푸시·테스트를 명시적으로 요청했다. 전체 Python 514개(53.140초), Node 278개, GitHub Actions와 같은 Node 파일 격리 9개 파일을 다시 검증해 모두 통과했다. 커밋·Pages 공개 결과는 아래 후속 확인에 기록한다. **운영 API·터널은 재시작하지 않았으며 DB v12→v14도 운영에 적용하지 않았다.** 실제 개인 녹음이나 외부 유료 ASR/LLM 호출은 이번 검증에 사용하지 않았다.
...
새 유지보수 worker는 명시적 테스트 Settings에서는 꺼지며 운영 factory에만 연결된다. 시작 중 실패해도 이미 시작한 worker를 정리한다. 종료 대기 예산은 worker 합계 18초이며, CLOVA/client close 시간까지 포함한 절대 18초 보장은 아니다. 운영 DB v12→v14 마이그레이션은 아직 하지 않았고 합성 이전 DB 보존 회귀로 검증했다.

서버 반영 전 할 일: 수업 중인지 확인 → 최신 암호화 백업 → 승인된 서버 재시작 → DB 마이그레이션/비인증 접근/CORS/본인 인증 흐름 확인. 새 Pages 화면이 공개되어도 이전 API의 구간 재생·책갈피 경로는 아직 없으므로 서버 재시작 전에는 해당 새 기능을 사용할 수 없다. 복구 비밀키의 PC 밖 보관은 사용자가 수행해야 하며 완료했다고 가정하지 않는다.
