...
사용자가 좁은 창에서 글자가 한 글자씩 세로로 쌓이고, 입력 상태·기기 음성 보관 안내의 여백과 글자 크기가 다른 영역과 맞지 않는 문제의 수정을 요청했다. **웹 HTML/CSS의 구현·검증을 완료했고, 이후 사용자의 별도 배포 요청에 따라 웹 배포를 진행한다.** 이번 변경은 백엔드 재시작 없이 웹 배포로 반영할 수 있다. 기존 미추적 창업 소개 문서는 공개 대상에서 제외하고 그대로 보존했다. API·로컬 모델 서버 분리는 가능 여부만 검토했으며 구현하거나 운영 구조를 변경하지 않았다.
