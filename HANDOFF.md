...
사용자가 같은 PC에서 먼저 분리하도록 승인했다. API의 로그인·권한 검사·DB·Drive·CLOVA·AI 기능과 Qwen/강제 정렬기 프로세스를 나눴다. 모델 종류·음향 설정·기존 청크/문맥 알고리즘은 바꾸지 않았고, 다른 PC나 클라우드로 DB를 옮기지 않았다. **구현·격리 검증·운영 API 전환·독립 모델 준비와 짧은 실제 한국어 음성 확인을 완료했다. 최종 회귀·게시 결과는 아래에 기록한다.**
...
최종 검증: WSL 환경 보강까지 포함한 전체 Python **946개/339.484초**와 Node **491개/16파일**, 합계 **1,437개**가 통과했다(실패·오류·스킵0). Python 로그 `/tmp/stt-model-split-env-final.FIZ9mc/python-all.log`. 별도 집중/반복 검사와 Pages CI 검사는 중복 합산하지 않는다. Python/셸 구문·diff 및 스테이징 개인정보 검사도 통과했다. 위 실제4.8초 표본과 자동 테스트는 수시간 연속 수업·학교 Wi-Fi·동시 실사용·장시간 GPU 메모리 증가 시험을 대신하지 않는다. 이번 분리만으로 정확도나 지연 개선을 주장하지 않으며, 비공개 계정·키·음성·창업 소개 미추적 문서는 공개하지 않는다. 코드 게시·Pages 최종 확인은 이어서 기록한다.
