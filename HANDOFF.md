...
- 공개 Chromium의 데스크톱·390px에서도 페이지/연결 준비/빈 로그인 입력이 정상이고 모듈404·요청 실패·스크립트 예외가 없었다. 다만 기존 CSP의 대괄호 IPv6 host-source 두 항목을 Chromium이 거절해 콘솔 오류가 발생했다. 이미 무시되던 두 항목만 제거하고 정적 회귀를 추가했다. 일반 HTTPS 허용 등 정책을 넓히지 않았으며 실제 서비스 터널·localhost/127.0.0.1 연결은 유지한다. 이 HTML 수정과 배포 확인 문서를 후속 커밋으로 재게시하고 공개 브라우저를 다시 확인한다.
