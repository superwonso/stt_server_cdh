...
## 후속 통합 검증 체크포인트 (2026-09-05, 배포 준비)

사용자의 최신 요청은 무음 자동 휴식 제외, 영문 수업의 문장별·전체 문맥 번역 추가, 그동안 미배포였던 모든 변경의 검증과 배포다. 아래 두 `과거 구현 체크포인트`는 당시 기록이며 현재 상태는 이 절과 후속 배포 결과를 우선한다. 운영 재시작·푸시는 최종 검증 뒤 별도로 기록한다.
